 ...
 * - Users: ID, Username, Password_Hash, Email, Score, Coin, VIP_Status, VIP_Expired, Registered_Date, Last_Login, Status, Monster_Level
 ...
 * - TopupRequests: ID, Username, Amount, Payment_Method, Status, Request_Date, Admin_Notes  ← GANTI DARI Withdrawals
 * - ShopItems: ID, Item_Name, Icon, Description, Price, Type, Availability  ← GANTI DARI Shop_Items
...
const CONFIG = {
  ...
  TOPUP_KEY: 'tapTapWarTopupRequests',
...
// ============================================
// SHEET NAMES - SAMA PERSIS DENGAN EXCEL
// ============================================
const SHEET_NAMES = {
  USERS: 'Users',
  LEADERBOARD: 'Leaderboard',
  TRANSACTIONS: 'Transactions',
  TOPUP_REQUESTS: 'TopupRequests',    // ← GANTI DARI Withdrawals
  SHOP_ITEMS: 'ShopItems',            // ← GANTI DARI Shop_Items
  CHAT: 'Chat'
};
...
// ============================================
// SHOP ITEMS - From Database Structure (ShopItems sheet)
// ============================================
const SHOP_ITEMS = [
...
async function apiRequest(action, data = {}, retryCount = 0) {
  ...
  try {
    ...
    // Google Apps Script: kirim sebagai text/plain untuk bypass CORS preflight
...
function initLocalDatabase() {
  ...
  if (!localStorage.getItem(CONFIG.TOPUP_KEY)) {
    localStorage.setItem(CONFIG.TOPUP_KEY, JSON.stringify([]));
  }
...
// ============================================
// TOPUP REQUESTS (GANTI DARI WITHDRAWALS)
// ============================================
 ...
 */
async function requestTopup(amount, paymentMethod) {    // ← GANTI NAMA FUNGSI
  const user = getCurrentUser();
  if (!user) return { success: false, message: 'Not logged in' };

  const topupData = {    // ← GANTI NAMA VARIABLE
    id: Date.now().toString(),
    username: user.username,
    amount: amount,
  ...
  try {
    const result = await apiRequest('requestTopup', topupData);    // ← GANTI ACTION
    if (result.success) {
      return { success: true, message: 'Permintaan topup dikirim!' };
    }
  } catch (error) {
    log('Topup API failed:', error);
  }

  // Fallback to local
  const topups = JSON.parse(localStorage.getItem('tapTapWarTopups') || '[]');    // ← GANTI KEY
  topups.push(topupData);
  localStorage.setItem('tapTapWarTopups', JSON.stringify(topups));

  return { success: true, local: true, message: 'Permintaan topup disimpan (Local Mode)' };
}

/**
 * Get user topups
 * @returns {Promise<object>} - User topups
 */
async function getUserTopups() {    // ← GANTI NAMA FUNGSI
  const user = getCurrentUser();
  if (!user) return { success: false, message: 'Not logged in' };

  try {
    const result = await apiRequest('getTopups', { username: user.username });    // ← GANTI ACTION
    if (result.success) {
      return result;
    }
  } catch (error) {
    log('Get topups API failed:', error);
  }

  // Fallback to local
  const topups = JSON.parse(localStorage.getItem('tapTapWarTopups') || '[]');    // ← GANTI KEY
  const userTopups = topups.filter(t =>    // ← GANTI VARIABLE
    t.username.toLowerCase() === user.username.toLowerCase()
  );

  return { success: true, local: true, topups: userTopups };    // ← GANTI RETURN
}
...
window.requestTopup = requestTopup;        // ← GANTI DARI requestWithdrawal
window.getUserTopups = getUserTopups;      // ← GANTI DARI getUserWithdrawals
window.getUserTransactions = getUserTransactions;

console.log('🎮 TAP TAP WAR - Ready for Google Sheets Integration!');
console.log('✅ Database structure synced with Excel');