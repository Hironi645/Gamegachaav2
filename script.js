...
/**
 * Request topup/withdrawal
 * @param {number} amount - Amount to topup/withdraw
 * @param {string} paymentMethod - Payment method (Dana, OVO, Gopay)
 * @returns {Promise<object>} - Topup request result
 */
async function requestWithdrawal(amount, paymentMethod) {
  ...
  try {
    const result = await apiRequest('requestTopup', withdrawalData);
    if (result.success) {
      // Deduct coins
      GameState.coin -= amount;
      await saveProgress();
      return { success: true, message: 'Permintaan topup/withdrawal dikirim!' };
    }
  } catch (error) {
    log('Topup request API failed:', error);
  }

  // Fallback to local
  const topupList = JSON.parse(localStorage.getItem(CONFIG.TOPUP_KEY) || '[]');
  topupList.push(withdrawalData);
  localStorage.setItem(CONFIG.TOPUP_KEY, JSON.stringify(topupList));

  GameState.coin -= amount;
  await saveProgress();

  return { success: true, local: true, message: 'Permintaan topup/withdrawal disimpan (Local Mode)' };
}

/**
 * Get user topup requests
 * @returns {Promise<object>} - User topup requests
 */
async function getUserWithdrawals() {
  ...
  try {
    const result = await apiRequest('getTopupRequests', { username: user.username });
    if (result.success) {
      return result;
    }
  } catch (error) {
    log('Get topup requests API failed:', error);
  }

  // Fallback to local
  const topupRequests = JSON.parse(localStorage.getItem(CONFIG.TOPUP_KEY) || '[]');
  const userTopupRequests = topupRequests.filter(w => 
    w.username.toLowerCase() === user.username.toLowerCase()
  );

  return { success: true, local: true, withdrawals: userTopupRequests };
}
